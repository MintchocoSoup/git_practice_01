...
- status code
  ...
  - R : renamed
  - MM : X의 상태가 Modified
  - AM : X의 상태가 Added (X의 상태가 서로 다를 수 있다)
  - !! : ignored
    --ignored : 깃에 보이지 않는 파일의 상태까지 나타냄
  - "" : clean (아무것도 안뜸)

파일 1개 생성 -> 스테이징 -> 수정 -> 커밋 ->
파일 옮기기  -> 스테이징 (D -> R) // 2개의 파일이 서로 같다는 걸 알고 있다
파일 삭제하기 (D)
...
- 삭제하는 행위는 되돌릴 수 없음
- 커밋 -> 수정 -> 삭제 (오류)
  - git rm -f file //오류 없이 바로 삭제하고 싶을 때는 -f옵션
    
3. 언스테이징만 하고 파일은 내가 가지고 있고 싶을 경우
  git rm --cached file //내가 직접 빼는 것
...
##  파일 이름 변경하기
```bash
  git mv file1 file2 #file1 -> file2
